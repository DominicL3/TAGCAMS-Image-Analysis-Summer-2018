...
def hist_equalizer(month, name, min_max):
    if month == 18:
        folder = 'LaunchPlus18Month_PSFImages/'
    elif month == 6:
        folder = 'LaunchPlus6Month_PSFImages/'
    ...
    data = fits.getdata(folder + name)
    #norm = ImageNormalize(vmin=min_max[0], vmax=min_max[1], stretch=SqrtStretch())

    fig = plt.figure(figsize=(8,8))
    ax = fig.add_subplot(1,1,1)
    im = ax.imshow(data, vmin=min_max[0], vmax=min_max[1], cmap='gray')

    plt.show()
    return data

def datahist_equalizer(image_data, min_max):
    norm = ImageNormalize(vmin=min_max[0], vmax=min_max[1], stretch=SqrtStretch())

    fig = plt.figure(figsize=(8,8))
    ax = fig.add_subplot(1,1,1)
    im = ax.imshow(image_data, norm=norm, cmap='gray')

    plt.show()
    return image_data

#hist_equalizer(18, '20180320T003301S680_ncm_L0_V003.fits', [50,1000])
def pixel_offset(pointing_offset): # pointing offset should be in degrees
    offset_in_milliradians = 17.4533 * pointing_offset
    pixel_scale = 0.28 # milliradians per pixel

    return offset_in_milliradians/pixel_scale

def star_coordinates_imageJ(x_offset,y_offset):
    ''' Remember that in Python, the pointing offset means to go down
        In ImageJ, it means to go in the actual direction of subtraction (up) '''
    center_imageJ = np.array([1440.5, 1026.5])
    return center_imageJ + [pixel_offset(x_offset), pixel_offset(y_offset)]
...
def dark_correction(filename):
    ...
    #test_array[1::2,4::5] = 20
    return mean_darkpixel_DN_list, active_image

def mass_darkcorrection():
    for filename in glob.glob('LaunchPlus6Month_PSFImages/*'):
        ...
        fits.writeto('active dark corrected data/active ' + filename[27:], z, hdr)

def instrument_quaternions(list_of_filenames):
    quaternions_list = []
    for name in list_of_filenames:
        file = 'LaunchPlus18Month_PSFImages/' + name
        hdu = fits.getheader(file)
        beforetuple = []
        for i in np.arange(28,32):
            beforetuple.append(hdu[int(i)])
        aftertuple = tuple(beforetuple)
        quaternions_list.append(aftertuple)
    return quaternions_list # returns the instrument quaternions

# Generates list of dark-corrected images that have exposure times of 5s and 0.5s respectively
five_second_images = [img for img in glob.glob('active dark corrected data/*') if fits.getheader(img)['EXPTIME']==5]
half_second_images = [img for img in glob.glob('active dark corrected data/*') if fits.getheader(img)['EXPTIME']==0.5]
...
brights_5s = coordinates_to_tuples(pd.read_excel('Coordinates to Brightest Pixel.xlsx')['5s Brightest Pixel'].values)
brights_halfsec = coordinates_to_tuples(pd.read_excel('Coordinates to Brightest Pixel.xlsx')['0.5s Brightest Pixel'].values)

def brightest_star_subwindow(number_rows, number_columns, number_of_subplots_per_figure, vmin_vmax=[180,2500], break_on=False):
    ...
    half_window_range = 4 # if you want 8x8 window for the stars, set to 4
    current_figure = 0
...
globs = glob.glob('active dark corrected data/*')
...
def horizontal_vertical_PSF(filename, exposure=0.5, vmin_vmax=[180,2500], half_window_range=5, plot=False, lines=True): # plots intensity vs. pixel value of the horizontal and vertical profile
    ...
    if exposure == 0.5:
        ...
        assert fits.getheader(half_second_images[file_index])['EXPTIME'] == exposure
    elif exposure == 5:
        ...
        assert fits.getheader(five_second_images[file_index])['EXPTIME'] == exposure
    else:
...
washed_out_images = np.array([20,21,22,23,46,47,72,73,74,75])
...
#PSF_profiles_to_pdf('5s Diagonal PSFs.pdf',function = 'diagonal_PSFs',exposure=5)

def normalize_exposures(filename, cross_section='horizontal_vertical', half_window_range=5):
    '''Takes in a filenumber and returns the normalized PSF values based on exposure time
    ...
    assert exposure == 0.5 or exposure == 5, 'Image exposure time is not 0.5s or 5s.'
...
def subwindow_replacement(filenumber, diagonal_path, threshold=770, norm=True):
...
def plot_normalized_exposures_horizontalvertical(file_index,half_window_range=5, title_fontsize=12, axis_fontsize=10):
    ...
    fat_fig, [(ax_halfsec_subwindow, ax_5s_subwindow), (ax_halfsec_horizontal, ax_5s_horizontal), (ax_halfsec_vertical, ax_5s_vertical)] = plt.subplots(3,2, figsize=(9.5,9))
...
def plot_normalized_diagonal_exposures(file_index,half_window_range=5, title_fontsize=12, axis_fontsize=10):
    ...
    fig, [(ax_halfsec_subwindow, ax_fivesec_subwindow), (ax_halfsec_updiagonal, ax_fivesec_updiagonal), \
    (ax_halfsec_downdiagonal, ax_fivesec_downdiagonal)] = plt.subplots(3,2, figsize=(12,9))
    ...
    # The half second subwindow stuff
    ...
    ax_halfsec_subwindow.set_ylim(yback_halfsec, yfront_halfsec) # inverted so that origin is upper left
    ax_halfsec_subwindow.set_xlim(xfront_halfsec, xback_halfsec)
    ax_halfsec_subwindow.set_title(str(half_second_images[file_index][27:]), fontsize=title_fontsize, pad=10)
    ...
    ax_fivesec_subwindow.set_title(str(five_second_images[file_index][27:]), fontsize=title_fontsize, pad=10)
    ...
    ax_halfsec_updiagonal.plot(halfsecond_normalized_ascending[0], halfsecond_normalized_ascending[1])
    ax_halfsec_updiagonal.set_title('0.5s Exposure Normalized Ascending PSF', fontsize=title_fontsize)
    ax_halfsec_updiagonal.set_xlabel('Distance from Center', fontsize=axis_fontsize)
    ax_halfsec_updiagonal.set_ylabel('DN/sec', fontsize=axis_fontsize)
    ax_halfsec_updiagonal.set_ylim(top=5200)

    # plotting normalized 0.5s descending data
    ax_halfsec_downdiagonal.plot(halfsecond_normalized_descending[0], halfsecond_normalized_descending[1])
    ax_halfsec_downdiagonal.set_title('0.5s Exposure Normalized Descending PSF', fontsize=title_fontsize)
    ax_halfsec_downdiagonal.set_xlabel('Distance from Center', fontsize=axis_fontsize)
    ax_halfsec_downdiagonal.set_ylabel('DN/sec', fontsize=axis_fontsize)
    ax_halfsec_downdiagonal.set_ylim(top=5200)
    ...
    ax_fivesec_updiagonal.plot(fivesecond_normalized_ascending[0], fivesecond_normalized_ascending[1])
    ax_fivesec_updiagonal.set_title('5s Exposure Normalized Ascending PSF', fontsize=title_fontsize)
    ax_fivesec_updiagonal.set_xlabel('Distance from Center', fontsize=axis_fontsize)
    ax_fivesec_updiagonal.set_ylabel('DN/sec', fontsize=axis_fontsize)
    ax_fivesec_updiagonal.set_ylim(top=850) # the normalized values for 5s cap out at around 800

    ax_fivesec_downdiagonal.plot(fivesecond_normalized_descending[0], fivesecond_normalized_descending[1])
    ax_fivesec_downdiagonal.set_title('5s Exposure Normalized Descending PSF', fontsize=title_fontsize)
    ax_fivesec_downdiagonal.set_xlabel('Distance from Center', fontsize=axis_fontsize)
    ax_fivesec_downdiagonal.set_ylabel('DN/sec', fontsize=axis_fontsize)
    ax_fivesec_downdiagonal.set_ylim(top=850) # the normalized values for 5s cap out at around 800
...
def PDF_peak_remover(output_name, cross_section='horizontal_vertical', half_window_range=5, dpi=800):
    ...
    with PdfPages(output_name) as pdf:
        i = 1
        for filenumber in [i for i in np.arange(len(five_second_images)) if i not in washed_out_images]:
            active_figure = replace_peaks(filenumber,cross_section,half_window_range=half_window_range,plot=True)[-1]
...
#PDF_peak_remover('Diagonal PSFs peaks removed.pdf',cross_section='diagonal')
...
DOY103_5s_images = [img for img in five_second_images if img in globs[58:] and img not in np.array(five_second_images)[washed_out_images]]
DOY103_halfsec_images = [img for img in half_second_images if img in globs[58:] and img not in np.array(half_second_images)[washed_out_images]]
...
'''def PSF_regions(diagonal_path='left_down', cross_section='horizontal_vertical', DOY=100,camera='nft',gain=8, cmap='tab10',linewidth=1.5):
    ...
    return fig, replaced_horizontal, replaced_vertical'''
...
def PSF_regions(box_coordinates=None,diagonal_path='left_down', cross_section='diagonal', DOY=100,camera='nft',gain=8, cmap='tab10',linewidth=1.5,plot=True):
    '''Plots the horizontal and vertical PSFs for points that follow the specified diagonal_path'''
    from mpl_toolkits.axes_grid1.inset_locator import inset_axes
    ...
    # specifies which color the box around a region should be
    region_colors = {'center': 'tab:red', 'top left': 'orange', 'bottom left': 'orange',
                     'corner top right': 'tab:green', 'corner top left': 'tab:green',
                     'top right': 'tab:blue', 'bottom right': 'tab:blue',
                     'top': 'tab:green', 'bottom': 'tab:purple', 'left': 'orange', 'right':'tab:blue',
    ...
    if plot:
            ...
            for file_5sec, file_halfsec in zip(gain8_5s_images, gain8_halfsec_images):
                ...
                if region_halfsecond in diagonal_paths[diagonal_path] and region_fivesecond in diagonal_paths[diagonal_path]:
                    ...
                    if diagonal_path == 'left_up':
                        ...
                        ax.set_ylabel('Normalized Ascending PSF (1/s)',labelpad=14)
                        ...
                        ax.set_ylabel('Normalized Descending PSF (1/s)', labelpad=14)
        ...
        elif box_coordinates is not None:
            colored_rectangles_axis = plt.axes(box_coordinates)
        colored_rectangles_axis.imshow(np.zeros([2004,2752]),vmin=0,cmap='gray')
        colored_rectangles_axis.axis('off')
        for line_color, left_and_bottom_rectangle in zip(colors, rectangle_coordinates):
            colored_rect = patches.Rectangle(left_and_bottom_rectangle, 130,100, edgecolor=line_color,linewidth=1,facecolor='none')
        ...
        fig.suptitle('Camera: {0}   Gain: {1}    DOY: {2}\n PSF Direction: {3}'.format(camera.upper(), gain, DOY, directions[diagonal_path]))
        fig.tight_layout()
        #fig.subplots_adjust(left=.06, top=.9,wspace=.22)
        fig.subplots_adjust(left=.06,right=.98,top=.93,hspace=0)

        return fig, ax, replaced_horizontal, replaced_vertical, gain8_5s_images
    else:
        return gain8_5s_images, pointing_offset_coordinates,region_colors

def PSF_timecomparison(box_coordinates=[0.34,0.75,0.22,0.17],diagonal_path='left_down', DOY=100,camera='nft',gain=8, cmap='tab10',linewidth=1.5, half_window_range=5):
    fig, ax = PSF_regions(box_coordinates,diagonal_path, 'diagonal', DOY, camera, gain, plot=True)[0:2]
    ax.change_geometry(1,2,1)
    ax.set(xlabel='Distance from Center', title='+6 Months (DOY {})'.format(DOY))
    ax.set_ylim(top = 0.5)
    fig.tight_layout()
    fig.subplots_adjust(left=.06,right=.98,top=.92,hspace=0)
    fig.suptitle('')

    gain8_5s_images, pointing_offset_coordinates,region_colors = PSF_regions(diagonal_path='left_down', DOY=DOY, camera=camera, gain=gain,plot=False)
    directions = {'left_up': 'Northeast', 'left_down': 'Southeast', 'central': 'Central'}
    diagonal_paths = {'left_down': ['corner top left', 'top left', 'center', 'bottom right', 'corner bottom right'],
                      'left_up': ['corner bottom left', 'bottom left', 'center', 'top right', 'corner top right'],
                      'central': ['top', 'left', 'bottom', 'right', 'center']}
...
'''with PdfPages('Hybrid PSFs comparison trial 2/DOY100 NFT Southeast 6 month.pdf') as pdf:
    fig, fig_zoomedimages = PSF_timecomparison(DOY=100,camera='nft', diagonal_path='left_down')
    pdf.savefig(fig)
    pdf.savefig(fig_zoomedimages)'''

def diagonal_PSF_to_PDFs(output_name, cross_section='horizontal_vertical', DOY=100,camera='nft'):
    plt.ioff()
    fig_leftdown8 = PSF_regions(diagonal_path='left_down',cross_section=cross_section, DOY=DOY,camera=camera,gain=8)[0]
    fig_leftup8 = PSF_regions(diagonal_path='left_up',cross_section=cross_section,DOY=DOY,camera=camera,gain=8)[0]
    #fig_central8 = PSF_regions(diagonal_path='central',cross_section=cross_section,DOY=DOY,camera=camera,gain=8)[0]

    fig_leftdown10 = PSF_regions(diagonal_path='left_down',cross_section=cross_section,DOY=DOY,camera=camera,gain=10)[0]
    fig_leftup10 = PSF_regions(diagonal_path='left_up',cross_section=cross_section,DOY=DOY,camera=camera,gain=10)[0]
    #fig_central10 = PSF_regions(diagonal_path='central',cross_section=cross_section,DOY=DOY,camera=camera,gain=10)[0]

    with PdfPages(output_name) as pdf:
        pdf.savefig(fig_leftdown8)
        pdf.savefig(fig_leftup8)
        #pdf.savefig(fig_central8)
        pdf.savefig(fig_leftdown10)
        pdf.savefig(fig_leftup10)
        #pdf.savefig(fig_central10)
        plt.close('all')

    plt.ion()

#diagonal_PSF_to_PDFs('Diagonal PSF Regions DOY 100 NFT radiometric normalization.pdf', 'diagonal')
#diagonal_PSF_to_PDFs('Diagonal PSF Regions DOY 103 NCM radiometric normalization.pdf', 'diagonal', 103,'ncm')
#diagonal_PSF_to_PDFs('Diagonal PSF Regions DOY 103 NFT radiometric normalization.pdf', 'diagonal', 103,'nft')
...
def resample_image(filename, end_size=[1000,1000], order=3):
    ...
    if filename in np.array(half_second_images)[washed_out_images] or filename in np.array(five_second_images)[washed_out_images]:
        return 'Balkan lands'
...
def plot_shifted(filenumber, vmin_vmax=[0,2500]):
    title_labels = ['Original 0.5s Image', 'Original 5s Image', 'Resampled 0.5s Image', 'Resampled Image (Shifted)', 'Shifted Downsampled Image', 'Hybrid PSF']
    shifted_halfsecond_image, resampled_half, resampled_five, [original_half,original_five], downsampled = realignment(filenumber)
    PSFs = [original_half,original_five,resampled_half,shifted_halfsecond_image,downsampled, hybrid_PSF(filenumber)]
    figures = []
    for i in np.arange(len(title_labels)):
        fig,ax = plt.subplots(1,1,figsize=(8,6))
        ax.imshow(PSFs[i],vmin=vmin_vmax[0],vmax=vmin_vmax[1],cmap='gray')
        ax.grid(False), ax.set(xticks=[],yticks=[])
        ax.set_title(title_labels[i], fontname='Myriad Pro')
        fig.tight_layout()
        figures.append(fig)
...
rectangle_centers = {(1043,1467): 'center', (171,321):'corner top left', (171,2607):'corner top right',
...
def FOV_hybrids(DOY=100, camera='NFT', gain=8, box_dimensions=[180,180], vmin_vmax=[0,30]):
    ...
    for directionality in ['left_up', 'left_down', 'central']:
        [hybrids, filtered_regions_fivesec] = visualize_PSFs(diagonal_path=directionality,DOY=DOY,camera=camera,gain=gain,plot=False)[1]
    ...
    ax.set_title('Launch+6 Months \n DOY: {}'.format(DOY),fontsize=12), ax.set_title('Camera: {}'.format(camera), loc='left',fontsize=12), ax.set_title('Gain: {}'.format(gain), loc='right',fontsize=12)
    fig.tight_layout()
    return fig

def visualize_PSFs(diagonal_path='left_up', DOY=100, camera='NFT', gain=8, plot=True):
    '''Plots the horizontal and vertical PSFs for points that follow the specified diagonal_path'''
    #plt.style.use('bmh')
    # specifies the paths taken to plot the diagonal PSFs
    diagonal_paths = {'left_down': ['corner top left', 'top left', 'center', 'bottom right', 'corner bottom right'],
    ...
    # filters images by their DOY and camera
    if DOY == 100:
        filtered_half_images = [img for img in DOY100_halfsec_images if fits.getheader(img)['TCGAIN'] == gain and fits.getheader(img)['INSTRUME']==camera]
        filtered_five_images = [img for img in DOY100_5s_images if fits.getheader(img)['TCGAIN'] == gain and fits.getheader(img)['INSTRUME']==camera]
    elif DOY == 103:
        filtered_half_images = [img for img in DOY103_halfsec_images if fits.getheader(img)['TCGAIN'] == gain and fits.getheader(img)['INSTRUME']==camera]
        filtered_five_images = [img for img in DOY103_5s_images if fits.getheader(img)['TCGAIN'] == gain and fits.getheader(img)['INSTRUME']==camera]
    else:
        return 'Bad DOY'
    ...
    for i in np.arange(num_images):
        ...
        subplot_number +=1
    directions = {'left_up': 'Northeast', 'left_down': 'Southeast', 'central': 'Central'}
    ax_regions.set_title('Launch+6 Months\nDOY: {0}\nPSF Direction: {1}'.format(DOY,directions[diagonal_path]), loc='center', fontsize=12)
    ...
    fig.tight_layout()
    fig.subplots_adjust(bottom=0,top=0.91)
    if plot == False:
        plt.close(fig)
        plt.ion()
...
'''with PdfPages('PSFs across FOV DOY100.pdf') as pdf:
    plt.ioff()
    for gain in [8,10]:
        for camera in ['NCM','NFT']:
            figurine = FOV_hybrids(DOY=100, camera=camera, gain=gain)
            pdf.savefig(figurine,dpi=600)
            plt.close(figurine)
    plt.ion()'''

'''with PdfPages('Hybrid PSFs DOY103.pdf') as pdf:
    plt.ioff()
    for gain in [8,10]:
        for camera in ['NFT', 'NCM']:
            for path in ['left_up', 'left_down', 'central']:
                figurine = visualize_PSFs(diagonal_path=path, DOY=103, camera=camera, gain=gain)[0]
...
def test_MTF(list_of_indices):
    ...
    for i in np.arange(num_cols):
        ...
        ax_down[i].imshow(calculate_MTF(hybrid_PSF(list_of_indices[i])), cmap='gray')
...
def MTF_regions(diagonal='left_up', DOY=100, camera='NFT', gain=8, linestyle='o'):
    previous_fig, [hybrids, regions] = visualize_PSFs(diagonal, DOY, camera, gain, plot=False)
    ...
    ax1.set_title('Horizontal MTF')
    ax2.set_title('Vertical MTF')
    regions_ax.set_title('Camera: {}\nDirection: {}'.format(camera,directions[diagonal]), loc='left')
    regions_ax.set_title('MTF Regions\nLaunch+6 Months', loc='center')
    regions_ax.set_title('Gain: {}\nDOY:{}'.format(gain,DOY), loc='right')
    fig.tight_layout()
    return fig
...
def save_MTFs(dpi=500):
    ...
    for DOY in [100,103]:
        for camera in ['NFT', 'NCM']:
            if camera=='NCM' and DOY == 100:
                pass
            for gain in [8,10]:
                for direction in directions.keys():
                    f = MTF_regions(diagonal=direction, DOY=DOY, camera=camera, gain=gain)
                    f.savefig('MTF 6months/MTF DOY{} {} {} Gain {}'.format(DOY,camera,directions[direction],gain), dpi=dpi)
                    plt.close(f)
    plt.ion()
